...
import type {
	...
	Message,
	UIMessage,
	UseChatOptions as InternalUseChatOptions,
...
export type UseChatOptions = InternalUseChatOptions & {
	/**
	...
	onStreaming?: (message: UIMessage, data?: JSONValue[]) => void;

	onChatComplete?: (messages: UIMessage[]) => void;

	onSubmit?: (message: UIMessage) => void;
};
...
export function useChat({
	...
	const triggerRequest = useCallback(
		async (chatRequest: ChatRequest) => {
			void mutateStatus('submitted');
			void setError(undefined);

			const chatMessages = fillMessageParts(chatRequest.messages);

			const messageCount = chatMessages.length;
			const maxStep = extractMaxToolInvocationStep(chatMessages[chatMessages.length - 1]?.toolInvocations);
			...
			try {
				...
				void throttledMutate(chatMessages, false);

				const constructedMessagesPayload = sendExtraMessageFields
					? chatMessages
					: chatMessages.map(
							({ role, content, experimental_attachments, data, annotations, toolInvocations, parts }) => ({
								role,
								content,
				...
				await callChatApi({
					...
					restoreMessagesOnFailure() {
						if (!keepLastMessageOnError) {
							void throttledMutate(previousMessages, false);
						}
					},
					onResponse,
					onUpdate({ message, data, replaceLastMessage }) {
						void mutateStatus('streaming');

						void throttledMutate(
							[...(replaceLastMessage ? chatMessages.slice(0, chatMessages.length - 1) : chatMessages), message],
							false,
						);

						if (data?.length) {
							void throttledMutateStreamData([...(existingData ?? []), ...data], false);
						}

						if (onStreaming) {
							onStreaming(message, data);
						}
					},
					onToolCall,
					onFinish(message, options) {
						...
						if (onChatComplete) {
							onChatComplete(messagesRef.current);
						}
					},
					generateId,
					fetch,
	...
	const setMessages = useCallback(
		(messages: Message[] | ((messages: Message[]) => Message[])) => {
			...
			void mutate(messagesWithParts, false);
	...
	const setData = useCallback(
		(data: JSONValue[] | undefined | ((data: JSONValue[] | undefined) => JSONValue[] | undefined)) => {
			...
			void mutateStreamData(data, false);
	...
	const [controlledInput, setControlledInput] = useState(initialInput);
	...
	const handleSubmit = useCallback(
		async (
			...
			if (!controlledInput && !options.allowEmptySubmit) return;
			...
			const currentMessage = {
				...
				experimental_attachments: attachmentsForRequest.length > 0 ? attachmentsForRequest : undefined,
				parts: [{ type: 'text', text: controlledInput }],
			} as UIMessage;
			...
			const messages = messagesRef.current.concat(currentMessage);
			...
			void triggerRequest(chatRequest);
	...
	const addToolResult = useCallback(
		({ toolCallId, result }: { toolCallId: string; result: unknown }) => {
			...
			void mutate(currentMessages, false);

			// auto-submit when all tool calls in the last assistant message have results:
			const lastMessage = currentMessages[currentMessages.length - 1];
			if (isAssistantMessageWithCompletedToolCalls(lastMessage)) {
				void triggerRequest({ messages: currentMessages });
			}
		},
